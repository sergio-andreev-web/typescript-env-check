...
export class JsonObjectRule implements StringRule {
  readonly id = 'jsonObject';
  readonly description = 'Validates jsonObject values';
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid jsonObject value');
    }
  ...
  withOptions(options: RuleOptions): JsonObjectRule {
    return new JsonObjectRule({ ...this.options, ...options });
  }
