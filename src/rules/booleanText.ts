...
export class BooleanTextRule implements StringRule {
  readonly id = 'booleanText';
  readonly description = 'Validates booleanText values';
  readonly pattern = new RegExp("^(?:true|false)$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid booleanText value');
    }
  ...
  withOptions(options: RuleOptions): BooleanTextRule {
    return new BooleanTextRule({ ...this.options, ...options });
  }
