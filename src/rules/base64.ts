...
export class Base64Rule implements StringRule {
  readonly id = 'base64';
  readonly description = 'Validates base64 values';
  readonly pattern = new RegExp("^[A-Za-z0-9+/]+={0,2}$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid base64 value');
    }
  ...
  withOptions(options: RuleOptions): Base64Rule {
    return new Base64Rule({ ...this.options, ...options });
  }
