...
export class MimeTypeRule implements StringRule {
  readonly id = 'mimeType';
  readonly description = 'Validates mimeType values';
  readonly pattern = new RegExp("^[a-z]+/[a-z0-9.+-]+$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid mimeType value');
    }
  ...
  withOptions(options: RuleOptions): MimeTypeRule {
    return new MimeTypeRule({ ...this.options, ...options });
  }
