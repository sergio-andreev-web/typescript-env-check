...
export class UppercaseRule implements StringRule {
  readonly id = 'uppercase';
  readonly description = 'Validates uppercase values';
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid uppercase value');
    }
  ...
  withOptions(options: RuleOptions): UppercaseRule {
    return new UppercaseRule({ ...this.options, ...options });
  }
