...
export class HttpMethodRule implements StringRule {
  readonly id = 'httpMethod';
  readonly description = 'Validates httpMethod values';
  readonly pattern = new RegExp("^(?:GET|POST|PUT|PATCH|DELETE|HEAD|OPTIONS)$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid httpMethod value');
    }
  ...
  withOptions(options: RuleOptions): HttpMethodRule {
    return new HttpMethodRule({ ...this.options, ...options });
  }
