...
export class HexRule implements StringRule {
  readonly id = 'hex';
  readonly description = 'Validates hex values';
  readonly pattern = new RegExp("^[0-9a-fA-F]+$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid hex value');
    }
  ...
  withOptions(options: RuleOptions): HexRule {
    return new HexRule({ ...this.options, ...options });
  }
