...
export class DockerImageRule implements StringRule {
  readonly id = 'dockerImage';
  readonly description = 'Validates dockerImage values';
  readonly pattern = new RegExp("^[a-z0-9._/-]+(?::[a-zA-Z0-9._-]+)?$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid dockerImage value');
    }
  ...
  withOptions(options: RuleOptions): DockerImageRule {
    return new DockerImageRule({ ...this.options, ...options });
  }
