...
export class EnvKeyRule implements StringRule {
  readonly id = 'envKey';
  readonly description = 'Validates envKey values';
  readonly pattern = new RegExp("^[A-Z][A-Z0-9_]*$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid envKey value');
    }
  ...
  withOptions(options: RuleOptions): EnvKeyRule {
    return new EnvKeyRule({ ...this.options, ...options });
  }
