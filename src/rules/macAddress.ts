...
export class MacAddressRule implements StringRule {
  readonly id = 'macAddress';
  readonly description = 'Validates macAddress values';
  readonly pattern = new RegExp("^(?:[0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2}$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid macAddress value');
    }
  ...
  withOptions(options: RuleOptions): MacAddressRule {
    return new MacAddressRule({ ...this.options, ...options });
  }
