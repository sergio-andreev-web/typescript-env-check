...
export class DecimalRule implements StringRule {
  readonly id = 'decimal';
  readonly description = 'Validates decimal values';
  readonly pattern = new RegExp("^-?\\d+(?:\\.\\d+)?$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid decimal value');
    }
  ...
  withOptions(options: RuleOptions): DecimalRule {
    return new DecimalRule({ ...this.options, ...options });
  }
