...
export class AlphanumericRule implements StringRule {
  readonly id = 'alphanumeric';
  readonly description = 'Validates alphanumeric values';
  readonly pattern = new RegExp("^[A-Za-z0-9]+$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid alphanumeric value');
    }
  ...
  withOptions(options: RuleOptions): AlphanumericRule {
    return new AlphanumericRule({ ...this.options, ...options });
  }
