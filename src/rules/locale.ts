...
export class LocaleRule implements StringRule {
  readonly id = 'locale';
  readonly description = 'Validates locale values';
  readonly pattern = new RegExp("^[a-z]{2}(?:-[A-Z]{2})?$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid locale value');
    }
  ...
  withOptions(options: RuleOptions): LocaleRule {
    return new LocaleRule({ ...this.options, ...options });
  }
