...
export class HostnameRule implements StringRule {
  readonly id = 'hostname';
  readonly description = 'Validates hostname values';
  readonly pattern = new RegExp("^[A-Za-z0-9](?:[A-Za-z0-9.-]*[A-Za-z0-9])?$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid hostname value');
    }
  ...
  withOptions(options: RuleOptions): HostnameRule {
    return new HostnameRule({ ...this.options, ...options });
  }
