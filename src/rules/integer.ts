...
export class IntegerRule implements StringRule {
  readonly id = 'integer';
  readonly description = 'Validates integer values';
  readonly pattern = new RegExp("^-?\\d+$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid integer value');
    }
  ...
  withOptions(options: RuleOptions): IntegerRule {
    return new IntegerRule({ ...this.options, ...options });
  }
