...
export class CsvListRule implements StringRule {
  readonly id = 'csvList';
  readonly description = 'Validates csvList values';
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid csvList value');
    }
  ...
  withOptions(options: RuleOptions): CsvListRule {
    return new CsvListRule({ ...this.options, ...options });
  }
