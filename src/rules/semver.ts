...
export class SemverRule implements StringRule {
  readonly id = 'semver';
  readonly description = 'Validates semver values';
  readonly pattern = new RegExp("^\\d+\\.\\d+\\.\\d+(?:-[0-9A-Za-z.-]+)?$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid semver value');
    }
  ...
  withOptions(options: RuleOptions): SemverRule {
    return new SemverRule({ ...this.options, ...options });
  }
