...
export class HexColorRule implements StringRule {
  readonly id = 'hexColor';
  readonly description = 'Validates hexColor values';
  readonly pattern = new RegExp("^#[0-9a-fA-F]{6}$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid hexColor value');
    }
  ...
  withOptions(options: RuleOptions): HexColorRule {
    return new HexColorRule({ ...this.options, ...options });
  }
