...
export class UrlRule implements StringRule {
  readonly id = 'url';
  readonly description = 'Validates url values';
  readonly pattern = new RegExp("^https?://[^\\s/]+(?:/[^\\s]*)?$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid url value');
    }
  ...
  withOptions(options: RuleOptions): UrlRule {
    return new UrlRule({ ...this.options, ...options });
  }
