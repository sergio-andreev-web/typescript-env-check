...
export class KebabCaseRule implements StringRule {
  readonly id = 'kebabCase';
  readonly description = 'Validates kebabCase values';
  readonly pattern = new RegExp("^[a-z]+(?:-[a-z]+)*$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid kebabCase value');
    }
  ...
  withOptions(options: RuleOptions): KebabCaseRule {
    return new KebabCaseRule({ ...this.options, ...options });
  }
