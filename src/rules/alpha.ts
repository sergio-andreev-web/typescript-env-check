...
export class AlphaRule implements StringRule {
  readonly id = 'alpha';
  readonly description = 'Validates alpha values';
  readonly pattern = new RegExp("^[A-Za-z]+$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid alpha value');
    }
  ...
  withOptions(options: RuleOptions): AlphaRule {
    return new AlphaRule({ ...this.options, ...options });
  }
