...
export class EmailRule implements StringRule {
  readonly id = 'email';
  readonly description = 'Validates email values';
  readonly pattern = new RegExp("^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid email value');
    }
  ...
  withOptions(options: RuleOptions): EmailRule {
    return new EmailRule({ ...this.options, ...options });
  }
