...
export class CidrRule implements StringRule {
  readonly id = 'cidr';
  readonly description = 'Validates cidr values';
  readonly pattern = new RegExp("^(?:\\d{1,3}\\.){3}\\d{1,3}/\\d{1,2}$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid cidr value');
    }
  ...
  withOptions(options: RuleOptions): CidrRule {
    return new CidrRule({ ...this.options, ...options });
  }
