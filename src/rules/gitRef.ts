...
export class GitRefRule implements StringRule {
  readonly id = 'gitRef';
  readonly description = 'Validates gitRef values';
  readonly pattern = new RegExp("^[A-Za-z0-9._/-]+$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid gitRef value');
    }
  ...
  withOptions(options: RuleOptions): GitRefRule {
    return new GitRefRule({ ...this.options, ...options });
  }
