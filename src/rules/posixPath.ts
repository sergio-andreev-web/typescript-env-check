...
export class PosixPathRule implements StringRule {
  readonly id = 'posixPath';
  readonly description = 'Validates posixPath values';
  readonly pattern = new RegExp("^/(?:[^\\s/]+/?)*$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid posixPath value');
    }
  ...
  withOptions(options: RuleOptions): PosixPathRule {
    return new PosixPathRule({ ...this.options, ...options });
  }
