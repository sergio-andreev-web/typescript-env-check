...
export class IsoDateRule implements StringRule {
  readonly id = 'isoDate';
  readonly description = 'Validates isoDate values';
  readonly pattern = new RegExp("^\\d{4}-\\d{2}-\\d{2}$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid isoDate value');
    }
  ...
  withOptions(options: RuleOptions): IsoDateRule {
    return new IsoDateRule({ ...this.options, ...options });
  }
