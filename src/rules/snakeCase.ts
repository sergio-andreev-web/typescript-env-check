...
export class SnakeCaseRule implements StringRule {
  readonly id = 'snakeCase';
  readonly description = 'Validates snakeCase values';
  readonly pattern = new RegExp("^[a-z]+(?:_[a-z]+)*$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid snakeCase value');
    }
  ...
  withOptions(options: RuleOptions): SnakeCaseRule {
    return new SnakeCaseRule({ ...this.options, ...options });
  }
