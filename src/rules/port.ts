...
export class PortRule implements StringRule {
  readonly id = 'port';
  readonly description = 'Validates port values';
  readonly pattern = new RegExp("^\\d{1,5}$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid port value');
    }
  ...
  withOptions(options: RuleOptions): PortRule {
    return new PortRule({ ...this.options, ...options });
  }
