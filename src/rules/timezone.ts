...
export class TimezoneRule implements StringRule {
  readonly id = 'timezone';
  readonly description = 'Validates timezone values';
  readonly pattern = new RegExp("^[A-Za-z_]+/[A-Za-z_]+$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid timezone value');
    }
  ...
  withOptions(options: RuleOptions): TimezoneRule {
    return new TimezoneRule({ ...this.options, ...options });
  }
