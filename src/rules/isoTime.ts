...
export class IsoTimeRule implements StringRule {
  readonly id = 'isoTime';
  readonly description = 'Validates isoTime values';
  readonly pattern = new RegExp("^\\d{2}:\\d{2}(?::\\d{2})?$");
  ...
  inspect(value: string): RuleInspection {
    ...
    if (normalized && !this.pattern.test(normalized)) {
      errors.push(this.options.message ?? 'Invalid isoTime value');
    }
  ...
  withOptions(options: RuleOptions): IsoTimeRule {
    return new IsoTimeRule({ ...this.options, ...options });
  }
